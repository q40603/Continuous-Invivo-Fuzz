...
int main(int argc, char **argv_orig, char **envp) {
  ...
  if (get_afl_env("INVIVO")){ afl->fsrv.is_Invivo = true;}
  // afl->fsrv.is_Invivo = true;
  ...
  while (
    ...
    switch (opt) {
      ...
      case 'P': // invivo process id
        sscanf(optarg, "%d", &afl->fsrv.fsrv_pid);
        SAYF(cCYA "[Invivo_forksrv_id] %d\n", afl->fsrv.fsrv_pid);
        break;

      case 'r': // invivo master pid
        sscanf(optarg, "%d", &afl->fsrv.Invivo_master_pid);
        SAYF(cCYA "[Invivo_master_pid] %d\n", afl->fsrv.Invivo_master_pid);
        break;


      case 'y': // dictionary
        sscanf(optarg, "%d", &afl->Invivo_socket_shm_id);
        SAYF(cCYA "[dictionary ] %d\n", afl->Invivo_socket_shm_id);
        break;

      case 'H':
        sscanf(optarg, "%d", &afl->shm.shm_id);
        SAYF(cCYA "[shm_id] %d\n", afl->shm.shm_id);
        break;        
  ...
  if (!afl->in_dir || !afl->out_dir || show_help) {
    SAYF(cCYA "optind %d == argc %d, !afl->in_dir %d, !afl->out_dir %d, show_help %d\n", 
    optind ,argc,
    !afl->in_dir,
    !afl->out_dir,
    show_help);
  ...
  if (!afl->non_instrumented_mode && !afl->fsrv.qemu_mode &&
    ...
    if (map_size < new_map_size) {
      ...
      if(afl->fsrv.is_Invivo)
        afl_invivo_fsrv_start(&afl->fsrv, afl->argv, &afl->stop_soon,
