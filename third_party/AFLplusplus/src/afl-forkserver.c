...
int Invivo_connect_sock(char* sock_name){
    int addr_len;
    ...
    if ( (fd = socket(AF_LOCAL, SOCK_STREAM, 0)) == -1){
        FATAL("Can't create Invivo_handshake socket");
    }
    addr.sun_family = AF_LOCAL;
    strcpy(addr.sun_path, sock_name);
    addr_len = offsetof(struct sockaddr_un, sun_path) + strlen(addr.sun_path);
    if(connect(fd, (struct sockaddr*)&addr, addr_len) < 0){
       FATAL("Can't connect Invivo remote %s", sock_name);
    }
    return fd;
} 

void afl_invivo_fsrv_start(afl_forkserver_t *fsrv, char **argv,
  ...
  if (!be_quiet) { 
    ACTF("Spinning up the invivo fork server..."); 
    ACTF("forksrv_pd: %d\n", fsrv->fsrv_pid);
  }
  ...
  if(fsrv->fsrv_pid){
    ...
    ACTF("Setup Invivo %d sts channel...", fsrv->fsrv_pid);
    sprintf(sock_path, "/tmp/BSA_sts_%d.sock", fsrv->fsrv_pid);
    fsrv->fsrv_st_fd = Invivo_connect_sock(sock_path);
    ACTF("fd= %d", fsrv->fsrv_st_fd);
    ...
    // }
    ACTF("rlen: %d, map size = %d, real map size = %d\n", rlen, fsrv->map_size, fsrv->real_map_size);
    return;
    }
...
u32 afl_fsrv_get_mapsize(afl_forkserver_t *fsrv, char **argv,
  ...
  if(fsrv->is_Invivo){
    afl_invivo_fsrv_start(fsrv, argv, stop_soon_p, debug_child_output);
  }
  else{
...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
  ...
  if ((res = write(fsrv->fsrv_ctl_fd, &write_value, 4)) != 4) {
    ACTF("res: %d write ctl channel...", res);
    //ACTF("write(fsrv->fsrv_ctl_fd 1793 failed");
    if (*stop_soon_p) { return 0; }
    RPFATAL(res, "Unable to request new process from fork server (OOM?)");

  }
  //ACTF("write(fsrv->fsrv_ctl_fd 1793 success");
  fsrv->last_run_timed_out = 0;

  if ((res = read(fsrv->fsrv_st_fd, &fsrv->child_pid, 4)) != 4) {
    ACTF("res: %d read sts channel...", res);
    //ACTF("read(fsrv->fsrv_st_fd 1801 failed");
    if (*stop_soon_p) { return 0; }
    RPFATAL(res, "Unable to request new process from fork server (OOM?)");
  }
  //ACTF("read(fsrv->fsrv_st_fd 1801 success");
#ifdef AFL_PERSISTENT_RECORD
