...
if(invivo_mode){
          ...
          cc_params[cc_par_cnt++] = "-fexperimental-new-pass-manager";
          cc_params[cc_par_cnt++] =
              alloc_printf("-fpass-plugin=%s/afl-invivo-llvm-pass.so", obj_path);
  #else
  ...
  if (compiler_mode != GCC && compiler_mode != CLANG) {
    ...
    switch (bit_mode) {
      ...
      case 0:
        if(invivo_mode)
          cc_params[cc_par_cnt++] =
              alloc_printf("%s/invivo-rt.o", obj_path);  
...
#undef NATIVE_MSG
  ...
  ptr = find_object("invivo-rt.o", argv[0]);

  if(!ptr){
        FATAL("Unable to find 'invivo-rt.o'. Please make sure it exists.");    
  }
  ck_free(ptr);
  if (debug) { DEBUGF("rt=%s obj_path=%s\n", ptr, obj_path); }
