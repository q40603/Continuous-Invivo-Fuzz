...
if test -f ${DESTDIR}/root/eval/httpd-2.4.25/install/bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/root/eval/httpd-2.4.25/install/bin/apr-1-config
   APU_CONFIG=${DESTDIR}/root/eval/httpd-2.4.25/install/bin/apu-1-config
else
...
test -n "/root/eval/httpd-2.4.25/install/bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/root/eval/httpd-2.4.25/install/build"
