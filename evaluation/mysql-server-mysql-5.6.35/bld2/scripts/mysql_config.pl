...
#        handle -lm -lssl -lcrypto -ldl -lcrypt -laio  and other strings inserted that might contain
...
my $ldata   = '/root/eval/mysql-server-mysql-5.6.35/bld2/data';
my $execdir = '/root/eval/mysql-server-mysql-5.6.35/bld2/mysql/bin';
my $bindir  = '/root/eval/mysql-server-mysql-5.6.35/bld2/mysql/bin';

# ----------------------------------------------------------------------
# If installed, search for the compiled in directory first (might be "lib64")
# ----------------------------------------------------------------------

my $pkglibdir = fix_path('/root/eval/mysql-server-mysql-5.6.35/bld2/mysql/lib',"libmysql/relwithdebinfo",
                         "libmysql/release","libmysql/debug","lib/mysql","lib");

my $pkgincludedir = fix_path('/root/eval/mysql-server-mysql-5.6.35/bld2/mysql/include', "include/mysql", "include");
...
my $flags;
$flags->{libs} =
  [@ldflags,@lib_opts,'','-lm -lssl -lcrypto -ldl ','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','-lm -lssl -lcrypto -ldl -lcrypt -laio ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','','-lm -lssl -lcrypto -ldl -lcrypt -laio ','',''];

$flags->{include} = ["-I$pkgincludedir"];
$flags->{cflags}  = [@{$flags->{include}},split(" ",'/root/BSA_test/third_party/afl-2.52b/afl-BSA-llvm-rt.o -Xclang -load -Xclang /root/BSA_test/third_party/afl-2.52b/afl-BSA-llvm-pass.so -mllvm -config=/root/eval/mysql-server-mysql-5.6.35/config -mllvm -level=2 -pthread   -O3 -g -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
$flags->{cxxflags}= [@{$flags->{include}},split(" ",'/root/BSA_test/third_party/afl-2.52b/afl-BSA-llvm-rt.o -Xclang -load -Xclang /root/BSA_test/third_party/afl-2.52b/afl-BSA-llvm-pass.so -mllvm -config=/root/eval/mysql-server-mysql-5.6.35/config -mllvm -level=2 -pthread   -O3 -g -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
