...
#        handle  -lpthread -lm -lssl -lcrypto -ldl -lcrypt -laio  and other strings inserted that might contain
...
my $ldata   = '/root/eval/mysql-server-mysql-5.6.35/bld/data';
my $execdir = '/root/eval/mysql-server-mysql-5.6.35/bld/mysql/bin';
my $bindir  = '/root/eval/mysql-server-mysql-5.6.35/bld/mysql/bin';

# ----------------------------------------------------------------------
# If installed, search for the compiled in directory first (might be "lib64")
# ----------------------------------------------------------------------

my $pkglibdir = fix_path('/root/eval/mysql-server-mysql-5.6.35/bld/mysql/lib',"libmysql/relwithdebinfo",
                         "libmysql/release","libmysql/debug","lib/mysql","lib");

my $pkgincludedir = fix_path('/root/eval/mysql-server-mysql-5.6.35/bld/mysql/include', "include/mysql", "include");
...
my $flags;
$flags->{libs} =
  [@ldflags,@lib_opts,'',' -lpthread -lm -lssl -lcrypto -ldl ','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'',' -lpthread -lm -lssl -lcrypto -ldl -lcrypt -laio ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','',' -lpthread -lm -lssl -lcrypto -ldl -lcrypt -laio ','',''];
